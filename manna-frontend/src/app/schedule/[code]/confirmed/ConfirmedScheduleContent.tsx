...
import { IoClose, IoCalendarClear, IoTimeOutline, IoLocationOutline } from "react-icons/io5";
...
export default function ConfirmedScheduleContent({
  ...
  // Guest용 확정 정보 조회
  const { data: confirmInfo, isLoading, error } = useGuestConfirmInfo(code, participantNo, true);
      ...
      <div className="min-h-screen bg-gray-50 -mx-16">
        <div className="px-16 pb-32">
          <header className="h-56 flex items-center justify-between">
            ...
            <button onClick={handleClose} className="w-24 h-24 flex items-center justify-center">
              <IoClose className="w-24 h-24 text-gray-600" />
            </button>
          </header>
          <div className="flex flex-col items-center justify-center py-32">
            <Spinner size="md" />
            <p className="mt-12 text-body14 text-gray-500">정보를 불러오는 중...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-gray-50 -mx-16">
        <div className="px-16 pb-32">
          ...
          <div className="bg-white rounded-[8px] border border-gray-200 p-16 text-center">
            <p className="text-body16 text-gray-600">일정 정보를 불러올 수 없습니다.</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gray-50 -mx-16">
        <div className="px-16 pb-32">
          ...
          <div className="bg-white rounded-[8px] border border-gray-200 p-16 text-center">
            <p className="text-body16 text-gray-600">아직 확정된 일정이 없습니다.</p>
          </div>
        </div>
      </div>
    );
  ...
  const filteredParticipant =
    participantNo && confirmInfo.confirmed_participants
      ? confirmInfo.confirmed_participants.find((p) => p.no === participantNo && p.confirmed_unit)
      : null;
    ...
    <div className="min-h-screen bg-gray-50 -mx-16">
      <div className="px-16 pb-32">
        ...
        <div className="space-y-16">
          ...
          <ScheduleInfoCard schedule={{ ...scheduleInfoCardData, expiry_datetime: null }} />

          {/* 그룹 확정 정보 */}
          {isCommon && confirmInfo.confirmed_unit && (
            <div className="bg-white rounded-[8px] border border-gray-200 p-16">
              <p className="text-subtitle16 text-gray-800 mb-16">모임 날짜가 정해졌어요!</p>
              ...
              <div className="space-y-8 mb-16">
                ...
                {confirmInfo.detail_address && (
                  <div className="flex items-center gap-8">
                    ...
                    <p className="text-body14 text-gray-700">{confirmInfo.detail_address}</p>
                  </div>
                )}
              </div>
              ...
              {confirmInfo.is_participant_visible && (
                <>
                  {confirmInfo.participants && confirmInfo.participants.length > 0 && (
                    <div className="mb-12">
                  ...
                  {/* 미참여자 목록 */}
                  {confirmInfo.non_participants && confirmInfo.non_participants.length > 0 && (
                    <div>
          ...
          {isIndividual && filteredParticipant?.confirmed_unit && (
            <div className="bg-white rounded-[8px] border border-gray-200 p-16">
              <p className="text-subtitle16 text-gray-800 mb-16">
                <span className="text-blue-500">{filteredParticipant.name}</span>
                님의 일정이 확정되었어요!
              </p>
              ...
              <div className="space-y-8">
                ...
                {filteredParticipant.confirmed_unit.time && (
                  <div className="flex items-center gap-8">
                    <IoTimeOutline className="w-20 h-20 text-blue-500" />
                    <p className="text-subtitle16 text-gray-900">
                      {formatTimeDisplay(filteredParticipant.confirmed_unit.time)}
                    </p>
                  </div>
                )}
                {confirmInfo.detail_address && (
                  ...
                  .map((participant) => (
                    ...
                    >
                      <p className="text-subtitle16 text-gray-800 mb-12">
                        <span className="text-blue-500">{participant.name}</span>
                        님의 일정
                      </p>
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center gap-8">
                          ...
                          <p className="text-body14 text-gray-700">
                            {formatToMonthDate(participant.confirmed_unit!.date)} (
                            {formatToKoreanDay(participant.confirmed_unit!.date)})
                          </p>
                        </div>
                        {participant.confirmed_unit!.time && (
                          <div className="flex items-center gap-8">
                            <IoTimeOutline className="w-20 h-20 text-blue-500" />
                            <p className="text-body14 text-gray-700">
                              {formatTimeDisplay(participant.confirmed_unit!.time)}
                            </p>
                          </div>
                        )}
                      </div>
          ...
          {isIndividual && !participantNo && !confirmInfo.is_participant_visible && (
            <div className="bg-white rounded-[8px] border border-gray-200 p-16 text-center">
              <p className="text-body14 text-gray-600">응답자 정보가 비공개로 설정된 일정입니다.</p>
            </div>
          )}
        </div>
      </div>
